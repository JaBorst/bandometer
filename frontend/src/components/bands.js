...
class BandDesc extends React.Component {
  constructor(props){
    super(props);
    this.state = {
      "band": props.item,
  ...
  render(){
      return (
       ...
       <a className="card-block stretched-link text-decoration-none" href={URL + "bands/" + this.props.id}>
        <h5 className="card-title">{this.props.name}</h5>({this.props.location})
